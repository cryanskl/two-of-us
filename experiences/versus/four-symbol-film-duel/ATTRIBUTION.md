...
标签、生产 UI、CSS 票根与幕布视觉、可访问交互和测试均由本仓库独立设计和
编写。实现没有参考、复制、修改、链接或打包任何第三方猜电影游戏、开源项目、
题库、剧情简介、台词、海报、剧照、Logo、字体、音频、视频或厂商 Emoji
图像。

本目录没有第三方运行时依赖、第三方代码或第三方资产，因此没有需要随作品
再分发的第三方许可证正文、版权声明或 notice。

视觉确认阶段生成的两张概念 PNG 只存在于仓库 `docs/assets/`，用于确认构图、
气质、色彩和信息层级；运行页没有引用、裁切、描摹或打包这些图，也没有复制
图中的生成式字形。生产界面只使用语义化 DOM、受控 CSS 色面和本机字体。
