...
本作品的玩法、代码、文案、测试与界面均为独立原创实现；运行背景由 OpenAI ImageGen 为本项目生成，没有输入第三方图片。开源项目与平台规范只用于研究公开技术思想，不是运行依赖，也没有复制其源码或素材。固定版本、许可证、无许可证排除项目与完整零复制边界见 [`ATTRIBUTION.md`](./ATTRIBUTION.md)。
