...
本作品沿用 [`cryanskl/html_lovetree`](https://github.com/cryanskl/html_lovetree) 的代码、文案结构与素材组合，属于“原项目内容迁移”，不是本仓库独立实现。源仓库和当前仓库均未声明统一开源许可证，因此不能把“公开可见”理解为“可以任意复制、修改和再分发”。

运行目录内还包含旧版 jQuery、Jscex 等第三方脚本，其文件头或源码注释保留了部分上游线索，但当前来源链和许可证并不完整；在确认授权或完成干净重写前，不应把它作为可自由再分发的开源模板。背景音乐是商业录音，需由使用者自行确认授权。
