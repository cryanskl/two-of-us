...
当 `composeMotif()` 允许返回 8–16 个音时，齿窗节点数与活动索引都必须由 `motif.length` 派生，不能借用“每圈四格”这个物理常量。第 10 个音应高亮索引 9，而不是在四个位置里循环。

本作把准备者可贡献的 5–10 行留在 `config.js`：可在合法 note ID 中重排专属动机；返回 `null`、抛错或生成非法结果都会回退到仓库原创旋律。这样个性化入口不破坏状态机和本地直开契约。
