...
- health 的 `contentIdentity` 与本次启动器算出的内容身份完全一致；
...
校验内容身份意味着重新读遍整个仓库并重新哈希，代价随作品与素材增长（当前约 2 秒）。门户只需要地址、二维码和 Node 版本，不看这个字段，因此这不是 `/api/health` 的默认工作：

| 请求 | 行为 |
| --- | --- |
| `GET /api/health` | 立即返回，**不含** `contentIdentity`，不做任何全仓读取 |
| `GET /api/health?verify=content-identity` | 重新计算并返回 `contentIdentity`：与启动时一致则为该身份，签出已变化或读取失败则为 `null` |

只有启动器的复用探测会带上这个查询，也只有它承担那次重算。不索取的调用者拿不到这个字段，而不是拿到一个未经校验、看起来却像已校验的值——两者对复用判定同样是「不可复用」，但前者不会让人误读。并发的校验请求共享同一次进行中的计算。
