...
| Gate | 结果 |
| --- | --- |
| 红测 | 修复前 1 pass / 2 fail，正确指出共享声明缺固定来源 |
| 依赖归因定向测试 | 3/3 通过 |
| 依赖归因 + 入口合同 + vendor 定向测试 | 70/70 通过 |
| `npm test` / 同一测试集合同步等待 | 112 个测试文件，2304/2304 通过，0 fail |
| `npm run verify` | 通过；58 个入口、50 A、8 非 A、1 个能力声明 |
| 固定网络链接 | 三项依赖的源码/许可证共 6 个 URL，6/6 HTTP 200 |
| Markdown 本地链接 | 851 个文件、2347 个本地链接、0 缺失 |
| `git diff --check 1169d67..HEAD` | 通过 |

仓库 `scripts/run-tests.mjs` 在当前命令执行环境中会在子 Node test runner 结束前先
返回，因此本轮先执行了要求的 `npm test`，再对它收集到的同一组 112 个测试文件
同步等待并保存 TAP 汇总，以真实子进程退出码和 2304/2304 结果作为全仓测试证据。
