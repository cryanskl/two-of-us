...
| 本次是否重跑全仓测试 | 否；本次只创建调度文档 |
| 工作树提示 | 创建本看板前已有未跟踪的 `docs/goal.md`，不得擅自覆盖 |
...
以下项目已有较完整的前置研究或规格，适合总控在确认无重复、文档仍有效且许可证边界清楚后，从中选择 4 个互不重叠的项目派发：
...
当前已观察到的最大编号是 `253`。总控首次派发前应重新扫描 `docs/`，再为每个并行项目分配互不重叠的编号或编号区间。
