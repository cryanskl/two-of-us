...
> 状态：brainstorm、技术路线与产品边界冻结；里程碑 A、B1、B2 与 B3 Worker 协议完成，离线语音自检和 C 体验待实现
> 日期：2026-07-17
...
- B2c（已完成）：只对「我听见了」静态路径加入 COOP/COEP/OAC，既满足 pthread 又不改变既有作品；
- B3（已完成）：完成单实例模型复用、同源资源约束与结构化 Worker 协议；
- 用固定短 WAV 做断网自检；
- 独立提交。
