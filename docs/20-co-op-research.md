...
2026-07-17 已实现原创 C03“灯塔与小船”，作品名为“为你引航”。它采用灯塔显露/发现/通航许可与小船实际运动两套不可替代的输入通道，是 A 级单设备同屏作品。技术比较固定了 Matter.js、rot.js、boardgame.io 与 trylock/visibility 的版本和许可证，但不引入、复制或打包这些运行时；完整规则、视觉和来源边界见 [`29-lighthouse-passage-spec.md`](./29-lighthouse-passage-spec.md)，自动与 Chrome 验收见 [`31-lighthouse-passage-verification.md`](./31-lighthouse-passage-verification.md)。
