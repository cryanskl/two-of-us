...
| 双人合作 | [同机你画我猜](../experiences/co-op/hot-seat-pictionary/) 与 [双光点归巢](../experiences/co-op/twin-light-maze/) | 建立热座交接，并补齐同屏双控、联合状态 BFS 与压力板交棒样板 |
...
- 已收录：Love Tree、慢慢打开的信、爱的刮刮卡、今晚做什么、回到那一天、拼回这一刻、同机你画我猜、双光点归巢、反应力对决、心动拔河、心跳冲刺、同心解锁、隔屏画猜、和你一样、连心四子棋、密封猜拳；
- 已建立：`surprises / co-op / versus` 三类目录；
- 已建立：Node 18+ 统一安装、跨平台启动器、Socket.IO 房间协议和本地二维码；
- 已归档：原始 RAR 和与项目不匹配的 Azure workflow；
- 已执行：九个无第三方运行依赖的独立 A 级样板，其中爱的刮刮卡覆盖 Canvas 遮罩与可访问降级，今晚做什么覆盖 SVG 与等概率选择，拼回这一刻覆盖私人照片预处理与交换拼图状态机，心动拔河覆盖固定步长与公平双人输入，双光点归巢覆盖同屏双控、联合状态 BFS 与互相保持机关；
...
- 下一批：A 级牵线物理合作，或 D 级本地语音体验的能力验证；
