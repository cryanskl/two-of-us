...
同屏实时和单设备轮流最符合“本地点开即玩”。双设备实时同步通常会引入 WebSocket、WebRTC 或局域网服务，不适合第一阶段。
...
| 双人合作 | 同机你画我猜 | 用户明确需要；Canvas、计时、交接遮挡可复用于多种游戏 |
...
在线 demo 正常不代表本地可用。`fetch()` 本地 JSON、ES Modules、Service Worker、远程 CDN 和麦克风权限都可能让 `file://` 失败。每个正式收录作品都需要在双击入口后独立验收。
