...
NASA 将月相按顺序分为八档：新月、娥眉月、上弦月、盈凸月、满月、亏凸月、下弦月、残月；周期约 29.5 天。NASA 技术出版物给出的平均朔望月为 29.53059 天。
...
提交校准后只公开三项“已对齐 / 还差一点”，不显示目标数字。这样既避免盲猜没有反馈，也不会把纪念日直接写到界面；最终内容仍只在成功阶段创建 DOM。
...
调研对照了 [`mourner/suncalc`](https://github.com/mourner/suncalc)：当前 HEAD 为 `bbc91f689ede3ff7173011947d435b3fb6c0485d`，项目是 BSD-2-Clause 风格许可，可计算月亮照明比例、相位与位置。
...
1. 八相名称、周期和基准新月来自 NASA/USNO 天文资料；
2. SunCalc 仅作为“完整天文库为何不应进入首版”的对照，没有复制其公式、代码或素材；
3. 日期状态机、离线八相近似、拖拽量化、中文线索、视觉与测试均为本仓库原创；
