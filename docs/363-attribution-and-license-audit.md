...
>
> **后续结论（2026-07-28）**：本文列为 P0、也是当时唯一「公开分发阻断」的
> `love-tree` 已整体下架——作品目录、迁移代码、商业录音 `renxi.mp3` 与
> `archive/source-packages/html5-love-original.rar` 全部从仓库删除，由原创作品
> 「跟着光，长成一棵树」接替（见 [`394`](./394-light-grown-tree-spec.md)）。
> 下文关于该项的描述保留为审计当时的记录，不再代表仓库现状。
