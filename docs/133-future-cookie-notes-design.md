...
| 概念证据 | 生产决定 | 最终验收 |
| --- | --- | --- |
| 标题/副题 + 右侧进度的安静横栏 | `QuietHeader` 只有这三类文字和一根细线 | 通过；移动按冻结规则隐藏副题 |
| 三列固定为 when / where / together | DOM 始终按 `NOTE_IDS` 排列，不按打开顺序重排 | 通过；先开中间项截图与六种排列测试一致 |
| 已开与未开占位稳定 | 共用 FutureNote 容器尺寸，仅内容变体 | 通过；移动按内容自适应，无横向 layout shift |
| 左侧未开项有黄铜焦点环 | 原生 button + 3px focus-visible 环 | 通过；焦点转移与 forced-colors 已检查 |
| 移动三条签 + 唯一深红主按钮 | 390×844 压缩头部、签高与间距 | 通过；按钮高 48px、底边 537px |
| finale 只有一张长信 | `FinalLetter` 单容器，边注不做三张卡 | 通过；1280×800 与 390×844 均为单纸面 |
| 背景图不承载交互 | 原图直接背景，图失效用纯深墨蓝 | 通过；阻断背景后完整实玩 |
| 饼干写实但不承载文字 | 三态透明 sprite + 代码序号/标题/按钮 | 通过；正常/失败回退互斥，无双重渲染 |
| 深墨蓝/奶白/深红/黄铜的低饱和系统 | 使用本文 token，不加粉色渐变或背景染色 | 通过；计算样式和最终截图一致 |
| 文字层级为宋体标题 + 易读正文 | 本机字体栈、明确字号/字重/行高 | 通过；390px 标题单行，无孤字 |
| 生成图的两处文字错误 | 生产只渲染规格中的精确代码文字 | 通过；above-the-fold copy diff 无新增文案 |
...
- [x] 截取移动 collecting、移动 ready、桌面 finale 与移动 finale；桌面 collecting 用浏览器指标补证并记录 IAB 截图限制；
- [x] 在同一 QA 轮次对概念与最新实装截图同时使用 `view_image`；
- [x] 检查至少五项 fidelity、上首屏文案 diff、图标、字体、资产混合、响应式和动效；
- [x] 禁背景/禁图集、降低动效和强制颜色下规则仍完整；
- [x] 修复所有可修复视觉差异，偏差与工具限制记录于 [135-future-cookie-notes-verification.md](./135-future-cookie-notes-verification.md)，完成 agency sign-off。
