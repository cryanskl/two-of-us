...
本目录公开的是可复现、可验证的工程复盘，不是未经整理的内部操作流水。提交前必须遵守 [公开发布检查清单](./PUBLICATION.md)：

- 已修复缺陷应写清根因、解决方案和回归证据；
- 普通未修复问题可以公开，但必须明确状态、影响范围和当前限制；
- 可能影响安全、隐私或数据边界的问题，在修复和复核完成前不得公开可利用细节；
- 个人用户名、绝对路径、临时 IP、凭据和私人素材必须删除或使用语义占位符；
- 只与一次性工具环境有关、没有项目复用价值的记录应合并、归档或留在内部。
